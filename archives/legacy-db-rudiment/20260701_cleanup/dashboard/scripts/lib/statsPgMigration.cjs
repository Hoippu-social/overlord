...
const SQLITE_SOURCE_CLIENT_PATH = path.resolve(ROOT_DIR, 'src/generated/stats-sqlite-source-client');
...
function loadSourceClientCtor() {
    if (!fs.existsSync(SQLITE_SOURCE_CLIENT_PATH)) {
        throw new Error(`Stats SQLite source Prisma client is not generated yet: ${SQLITE_SOURCE_CLIENT_PATH}`);
    }
    return require(SQLITE_SOURCE_CLIENT_PATH).PrismaClient;
}

function createSourceClient(url) {
    const SqlitePrismaClient = loadSourceClientCtor();
